...
def get_pretrained_model_details():
    ...
    import time
    import gc
    models = {s.split('.')[0] for s in timm.list_models(pretrained=True) if '.' in s} #timm.list_models(pretrained=True)
    def count_parameters(model): return sum(p.numel() for p in model.parameters() if p.requires_grad)
    model_names, num_parameters, param_memories, forward_pass_memories, total_memories, input_image_sizes = [], [], [], [], [], []
    #ijk=0
    for m in models:
        try:
            ...
            tf_m = timm.create_model(m, num_classes=0, pretrained=False).cuda() # if pretrained, then the parameters are downloaded, which takes unnecessary time
            ...
            input_data = torch.randn((1, i_s[0], i_s[1], i_s[2])).cuda() #torch.randn((1, 3, 224, 224)).cuda()
            ...
            forward_pass_memory_checkpoint_cached = torch.cuda.memory_cached() / (1024 ** 3)
            ...
            forward_pass_memories += [forward_pass_memory_checkpoint] # - pre_forwardpass_memory]
        ...
        #if ijk > 10:
        #    break
    return pd.DataFrame({'Name':model_names, 'Parameter count': num_parameters, 'Forward pass memory': forward_pass_memories, 'Parameter memory': param_memories, 'Total memory':total_memories, 'Input image size': input_image_sizes}).sort_values(by='Total memory', ascending=True) # dict(sorted(dict_models.items(), key=lambda item: item[1]))
